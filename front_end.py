...
def handle_mode_3(user_id, memory_logger, data):
    ...
    if results.get("book"):
        book = results["book"]
        response_lines.append(f"📘 Book: {book.get('title')} by {book.get('author')}")
    if results.get("audiobooks"):
        response_lines.append("🎧 Audiobooks:")
...
def handle_mode_4(user_id, session, user_input):
    """Budget tracker mode (robust, memory-logger safe)."""
    memory_logger = session.get('memory_logger')

    def safe_log(action, value):
        """Logs safely without crashing if memory_logger is missing."""
        if memory_logger:
            try:
                memory_logger.log_interaction(action, value, tags=["budget"])
            except Exception as e:
                print("⚠️ Memory logger failed:", e)

    # Start generator if not created
    ...
    # Log the user input safely
    safe_log(user_input if user_input else "Start Budget Tracker", output)

    # 🔥 If generator returned structured progress data, pass it cleanly
    if isinstance(output, dict):
        return {
            "success": True,
            "response": output.get("message", ""),
            "goal_progress": output.get("goal_progress")
        }
...
def handle_mode_5(session, user_input, memory_logger):
    ...
    if not user_input or user_input.strip() == "":
        ...
        try:
            ...
            return {"success": False, "message": "⚠️ Enter a number 0-3.", "error": "Invalid input", "done": False, "answers": answers, "next_step": step}
...
def handle_mode_7(memory_logger):
    ...
    try:
        ...
        output = "✅ Voice GUI Assistant launched!"
    except Exception as e:
        output = f"❌ Failed to launch GUI: {e}"
    if memory_logger:
...
def handle_mode_10(session, user_input, data, memory_logger=None):
    """Reminder management mode — teen-friendly commands with normalized input."""
    ...
    if not user_input:
        return {"success": False, "response": "⚠️ Please enter a reminder command.", "tasks": reminder_manager.view_reminders()["tasks"]}

    # Split command from argument
    cmd_parts = user_input.split(" ", 1)
    raw_cmd = cmd_parts[0].lower()
    arg = cmd_parts[1].strip() if len(cmd_parts) > 1 else None

    # Teen-friendly command aliases
    cmd_aliases = {
        "add": "add",
        "create": "add",
        "view": "view",
    ...
    cmd = cmd_aliases.get(raw_cmd, None)
    ...
    if cmd == "add":
        if not arg:
            response.update({"success": False, "response": "⚠️ Reminder text required."})
        else:
            # Parse optional due time from data
            due_time = data.get("dueTime")
            # Default: 30 minutes from now if not specified
            if not due_time:
                from datetime import datetime, timedelta
                due_time = datetime.now() + timedelta(minutes=30)
            result = reminder_manager.add_reminder(arg, due_time)
            response.update(result)
            # Always include current tasks
            response["tasks"] = reminder_manager.view_reminders()["tasks"]


    elif cmd == "view":
        view_data = reminder_manager.view_reminders()
        response["tasks"] = view_data.get("tasks", [])
        response["response"] = "\n".join([t["text"] for t in response["tasks"]]) or "🔕 No reminders saved."

    elif cmd == "check_due":
        result = reminder_manager.check_due_reminders()
        response.update({"response": result["response"], "tasks": result["tasks"]})
        ...
        response.update({
            "success": False,
            "response": "⚠️ Unknown command. Use 'add', 'view', 'check_due', or 'clear'.",
            "tasks": reminder_manager.view_reminders()["tasks"]
        })

    # --- Log interaction ---
    if memory_logger:
        memory_logger.log_interaction(user_input, str(response), tags=["reminder", cmd or raw_cmd])

    return response

def handle_math_mode(user_input, memory_logger, mode="math_processing"):
    """
    Math processing mode — returns step-by-step output AND belief tags.
    """
    # 1️⃣ Define belief tags
    belief_tags = ["offer math tips", "show steps", "check calculations"]

    # 2️⃣ Reinforce beliefs in memory logger
    if memory_logger:
    ...
    # 3️⃣ Get strongest beliefs for display
    tasks = []
    if memory_logger:
    ...
    # 4️⃣ Generate step-by-step math output
    try:
        ...
        steps_output = f"⚠️ Error processing math input: {e}"

    # 5️⃣ Recommended mode (optional, fallback to current)
    recommended_mode = getattr(memory_logger.belief_model, "recommended_mode", lambda: mode)()

    # 6️⃣ Return structured JSON with both steps and tasks
    return {
        "success": True,
